...
| v3.6 | 2026-08-28 | Combo 连消奖励（r20）：锁定为触点，连消链递增（链内首消 combo0）、任意锁定清 0 行/No-line T-spin 断链归 0、hold/旋转(含踢墙)/软降/硬降不断链、普通+T-spin 可混链；comboBonus=50×combo×level（COMBO_BONUS_BASE=50，乘数取升级前 level）与普通消行分/T-spin 分三轴叠加恰各一次、不进等级进度；combo 为会话内存不入持久化、restart/OVER 归 0、onGameOver 总分=逐锁增量之和；取代 r19#AC-12 指示器 UI 归 P2 另立需求 | ✅ 已验收（tf-mtcomxpq-heissu：独立复跑七套全绿 119/24/23/2/ALL/367，qa-e2e 三处期望修正 950/950/+500 实打；AC-1~12 全过；0-diff 仅 game.js/verify-game.cjs/qa-e2e-jsdom.cjs；人工补测 3 项见任务夹 ACCEPTANCE） |
...
| v3.7 | 2026-08-28 | 特殊奖励 Toast（r21）：Combo/T-Spin 消行奖励即时 Toast——复用既有 LEVEL UP toast 子系统（#reward-toast 扩槽 + 独立 rewardTimer + toast-in-out 1600ms 复用 keyframes，零新 token）、单帧多轴合并 1 根（T-Spin 前·Combo 后）、全 0/No-line 静默、OVER/restart 0 残留、与 LEVEL UP 同帧双信息共存、aria-live=polite；数值只读引擎载荷（comboBonus 直读、tspinBonus 同函数派生恒等），纯展示零新计分路径，取代 r19#AC-12 指示器归 P2 | ✅ 已验收（tf-mtctcv9m-ou8v65：独立复跑八套全绿 119/24/28/2/15/ALL/395(+28 纯追加)/44；0-diff 仅 UI 五件套 engine/persist 0 行；D-1 已修复（restart 后首消 T-Spin 弹 +800，引擎 0 行）；AC-1~12 全过；人工补测 5 项见任务夹 ACCEPTANCE） |
| v3.8 | 2026-08-29 | Back-to-back 连续 Tetris/T-Spin 无间隙奖励倍率（r23）：第四计分轴 b2bBonus=400×升级前 level（B2B_BONUS_BASE=400 定值，不随链长递增）；资格=Tetris(4行)/T-Spin Full≥1行（复用 r18 tspinKind 判定）；b2bChain 布尔会话链态（finishLock 首行唯一出口：资格→on、非资格消行/0行/No-line→off，hold/旋转/软降/硬降不迁移，restart/OVER 归零）；四轴（普通/T-spin/combo/B2B）同帧叠加恰各一次、B2B 不进 lines/level；snapshot 暴露 b2bChain（恒 boolean）+b2bBonus（clearing 期非 null）；toast 三轴合并（r21 子系统扩展，B2B +N 载荷直读同源、全 0 静默、OVER/restart 清空、LEVEL UP 共存沿用）；会话隔离不入持久化 | ✅ 已验收（tf-mtd6yvyp-pih1ms：独立复跑七套全绿 130/24/29/2/15/ALL/429；QA 独立对抗抽查 46/46；AC-1~10 全过、AC-11 P2 另立；红线 audio/persist/index/style 0 行；人工补测 5 项见任务夹 ACCEPTANCE） |
...
- **人工补测汇总（环境限制非缺陷）**：各轮 QA-REPORT §6/§7 清单——音效听感/时序、B1~B9 听感视觉读屏、幽灵块暗色可辨识度/双分辨率、开关可访问性与即时性、快捷键时序、r16 真机触屏补测 R1~R10（真机能力检测/防滚动缩放/375×667+812×375 布局遮挡/刘海安全区/真实多指/长按速率手感/触屏笔记本混合设备/桌面零视觉变化/按键音效听感/D1 跨通道真机复验，详见 r16 QA-REPORT）；r23 新增：B2B toast 三轴排版/连发动效观感（真人浏览器触发连发）+ 真实读屏朗读新 B2B 文案（NVDA/VoiceOver）等；r24 新增：触控双簇三档 computed 尺寸（标准 64/96/56·紧凑 56/80/48·横屏轨 56/80/48，±1px）、320 零溢出与一屏零滚动实测量、M/L 恒玻璃 computed、横屏侧轨 [212,vw−212] 零遮挡截图对拍、C 渐隐默认态棋盘辨识度、外观组读屏走查、真机 safe-area/多指/FPS（工具 logs/teamflow/tf-mte60543-w71v24/qc-r24-browser.cjs 留档）；r26 新增：触控键帽/侧轨/2×2 选择器真机走查 9 项（S/M/L 竖屏双簇间距量值、Hold/旋转正圆观感、M/L 横屏恒底栏无侧轨、2×2 缩略可辨+选中清晰、safe-area/100dvh 工具栏不遮挡、多指六键独立生效、:active 三态、读屏语义、FPS 流畅度，详见 r26 QA-REPORT §6）；r34 新增：真机切后台/清后台补记时序（visibilitychange 双触发幂等 + 页面回收卸载补记）、进行中刷新不丢、暂停不计时抽查、旧 highScore 存档升级迁移、长挂机时长精度、读屏播报新增全局文本、横竖屏「全局」组布局不叠压（含 S 竖屏棋盘高度 526→≈458px 实机可玩性确认）
