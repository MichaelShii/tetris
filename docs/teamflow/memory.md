...
| **v2.6** | 2026-08-18 | 应用层持久化（tech 技术驱动改造，分支 `feat/persistence-localStorage2`，对照实验）：最高分本地持久化（AC-16）+ 音量/静音/幽灵/BGM 四设置刷新恢复（AC-10.5），`persist.js` UMD 层 + `verify-persist.cjs`；**不新增/不修改 PRD 功能 AC** | ❌ 打回（产品验收 2026-08-18，tf-mt317a5e-0iwvuy）：六套既有验证 + verify-persist 全绿（52/23/7/2/装配ALL/188+10/10），但均未走真实 localStorage 适配路径；**P1 缺陷 BUG-P1-1**——`persist.js` `createStorage` 探测用 `setItem/getItem/removeItem`、读写却调 `store.set/get/remove`（原生 localStorage 无此三法，内存 Map 风格）→ 真实浏览器 `available=true` 却读写全空、持久化完全静默失效（reload 最高分/设置全丢）；AC-16 / AC-10.5 核心交付在目标 `file://` 环境**无效**；架构 M3：分层/可选依赖/dispose 链对齐蓝图，但适配器「实现与契约偏离」= 适配器漂移，质量门禁拦截。**待修复**：`createStorage` 统一调用面为 `setItem/getItem/removeItem`（经薄层映射）+ verify-persist 补走真实 localStorage API 形状的对抗用例，修复后复跑全表 + 缺陷复现转绿 |
