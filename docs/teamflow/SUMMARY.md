...
| 文档 | 用途 | 关键章节 | 体积提示 |
|---|---|---|---|
| `prd/PRD.md` | **验收唯一依据**：AC-01~12 全集 + 数值规格（v2.2 幽灵块） | §2 用户故事与验收标准（AC 全集）、§5 数值规格（计分/等级/速度/音效/幽灵块） | 大（~200 行）：按需读 AC 相关条目 |
| `teamflow/memory.md` | **产品记忆**：迭代历史 + v2.2 幽灵块迭代记忆（已交付 ⚠️ 有条件通过）与待办 | 全文 | 小：PM/架构/QA 迭代时读 |
